...
| B2 Membership extraction | Completed | Canonical membership lifecycle routes run under `/v0/org/memberships/*`; legacy `/v0/auth/memberships/*` aliases delegate to canonical OrgAccount commands and emit canonical `org.membership.*` actions/events. Membership business logic and persistence are now owned by `orgAccount/membership` (`app/service.ts`, `infra/repository.ts`) and no longer depend on Auth service/repo internals. |
